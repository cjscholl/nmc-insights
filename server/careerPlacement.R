...
output$career_placement_plot <- renderPlotly({
  ...
  if(isTruthy(input$file2) && isTruthy(input$graduate_support_coalition_checkbox)) {
    df_coalition <- bind_rows(list(School=reactive_career_placement_df(), Coalition=reactive_coalition_career_placement_df()), .id="id")
    filtered_df = filter(df_coalition, between(`Middle School Graduating Class`, min, max))
    req(filtered_df$`Post Secondary Follow Up Year`)
    filtered_df.long <- pivot_longer(filtered_df, cols=5:8, names_to = "Type", values_to = "Percentage")
