...
output$enrollment_rate_plot <- renderPlotly({
  ...
  plot <- ggplot(data=filtered_df, aes(x=`School Year`, y=`Enrollment Percentage`, group=1)) +
    ylim(0, 100) +
  ...
  if(isTruthy(input$file2) && isTruthy(input$school_data_coalition_checkbox)) {
    df_coalition <- reactive_coalition_enrollment_historical_df()
    req(df_coalition)
    filtered_df_coalition = slice(df_coalition, min_index:max_index)
    
    plot <- plot + geom_line(data=filtered_df_coalition, aes(x=`School Year`, y=`Enrollment Percentage`), colour="#E15551") + 
      geom_point(data=filtered_df_coalition, aes(x=`School Year`, y=`Enrollment Percentage`), colour="#E15551", shape=21, fill="white")
  }
